...
void Draw::Word(std::string text, Screen screen, unsigned int textureSymbol, int x, int y)
{
  ...
  glEnable(GL_TEXTURE_2D);
      ...
      glPushMatrix();
        ...
        glLoadIdentity();
        float xf = -0.5 + (x)/(float)screen.width;
        float yf = -0.5 + (y)/(float)screen.height;
...
void debugInfo(std::string text, Screen screen, unsigned int textureSymbol)
{
